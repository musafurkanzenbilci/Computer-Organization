...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_IRMOVQ || hi0 == I_RMMOVQ || hi0 == I_MRMOVQ ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_ISUBQ);
	...
	s->pc = ftpc;
	break;
    /* Let's try to simulate isubq here! Sure lots of copy-paste... (DS) */
    case I_ISUBQ:
	if (!ok1) {
	    if (error_file)
		fprintf(error_file,
    ...
    argB = get_reg_val(s->r, lo1);
	val = compute_alu(A_SUB, argB, cval);
	set_reg_val(s->r, lo1, val);
	s->cc = compute_cc(A_SUB, argB, cval);
	s->pc = ftpc;
	break;
    default:
	if (error_file)
